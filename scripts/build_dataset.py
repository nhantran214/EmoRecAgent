...
def _log_config(logger, cfg) -> None:
    ...
    logger.info(
        "k_core=%s max_users=%s max_items=%s min_history=%s",
        d.k_core,
        d.max_users,
        d.max_items,
        d.min_history,
...
def main() -> int:
    ...
    review_path = Path(d.review_path)
    ...
    if d.max_users is not None or d.max_items is not None:
        ...
        logger.info("sampling skipped (no max_users/max_items caps) (%.1fs)", time.monotonic() - t_stage)

    logger.info("--- stage: chronological split ---")
    t_stage = time.monotonic()
