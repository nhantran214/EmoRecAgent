...
def test_salvage_truncated_pretty_printed_json() -> None:
    truncated = (
        '1 validation error for TripleSet\n  Invalid JSON: EOF while parsing a string '
        "at line 47 column 41 [type=json_invalid, input_value='{\\n  \"triples\": [\\n"
        '    {\\n      "aspect": "service",\\n      "opinion": "friendly",\\n'
        '      "sentiment": "positive"\\n    },\\n'
        '    {\\n      "aspect": "food",\\n      "opinion": "the clientele was a bit'
        "', input_type=str]"
    )
    out = salvage_triple_set_from_error(Exception(truncated))
    assert out is not None
    assert len(out.triples) == 1
    assert out.triples[0].aspect == "service"
