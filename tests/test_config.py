...
def test_default_config_has_tisasrec_align_section(monkeypatch):
    ...
    assert cfg.tisasrec_align.batch_size == 2048
...
def test_sports_category_overlay(monkeypatch):
    ...
    assert "Sports_and_Outdoors/tisasrec_align" in cfg.tisasrec_align.stage1_checkpoint_path
    # Hyperparameters unchanged from default
    assert cfg.data.k_core == 10
    assert cfg.tisasrec_align.hidden_units == 128
    assert cfg.tisasrec_align.rerank_pool_k == 100


def test_sports_align_overlay_same_method(monkeypatch):
    _set_env(monkeypatch)
    cfg = load_config("configs/categories/Sports_and_Outdoors_emorecagent_align.yaml")
    assert cfg.data.category == "Sports_and_Outdoors"
    assert cfg.eval.protocol == "user_batch"
    assert cfg.eval.aggregation == "user_mean"
    assert cfg.tisasrec_align.stage2_mode == "rerank"
...
def test_sports_stage1_baseline_overlay(monkeypatch):
    _set_env(monkeypatch)
    cfg = load_config(
        "configs/categories/Sports_and_Outdoors_emorecagent_stage1_baseline.yaml"
    )
    assert cfg.data.category == "Sports_and_Outdoors"
    assert cfg.tisasrec_align.stage1_only is True
    assert cfg.eval.protocol == "user_batch"
    # Same as Beauty stage1 baseline: stage2_mode unused when stage1_only
    beauty = load_config("configs/emorecagent_stage1_baseline.yaml")
...
def test_sports_paths_do_not_touch_beauty(monkeypatch):
    ...
    beauty = load_config("configs/default.yaml")
...
def test_toys_category_overlay(monkeypatch):
    ...
    assert "Toys_and_Games/tisasrec_align" in cfg.tisasrec_align.stage1_checkpoint_path
    assert cfg.data.k_core == 10
    assert cfg.tisasrec_align.hidden_units == 128
    assert cfg.tisasrec_align.rerank_pool_k == 100


def test_toys_align_overlay_same_method(monkeypatch):
    _set_env(monkeypatch)
    cfg = load_config("configs/categories/Toys_and_Games_emorecagent_align.yaml")
    assert cfg.data.category == "Toys_and_Games"
    assert cfg.eval.protocol == "user_batch"
    assert cfg.eval.aggregation == "user_mean"
    assert cfg.tisasrec_align.stage2_mode == "rerank"
...
def test_toys_stage1_baseline_overlay(monkeypatch):
    _set_env(monkeypatch)
    cfg = load_config(
        "configs/categories/Toys_and_Games_emorecagent_stage1_baseline.yaml"
    )
    assert cfg.data.category == "Toys_and_Games"
    assert cfg.tisasrec_align.stage1_only is True
    assert cfg.eval.protocol == "user_batch"
...
def test_yelp_category_overlay(monkeypatch):
    ...
    assert "Yelp/tisasrec_align" in cfg.tisasrec_align.stage1_checkpoint_path
    assert cfg.data.k_core == 20
    assert cfg.tisasrec_align.hidden_units == 128
    assert cfg.tisasrec_align.rerank_pool_k == 100


def test_yelp_align_overlay_same_method(monkeypatch):
    _set_env(monkeypatch)
    cfg = load_config("configs/categories/Yelp_emorecagent_align.yaml")
    assert cfg.data.category == "Yelp"
    assert cfg.eval.protocol == "user_batch"
    assert cfg.eval.aggregation == "user_mean"
    assert cfg.tisasrec_align.stage2_mode == "rerank"
    assert cfg.tisasrec_align.use_hash_encoder is True
    assert cfg.tisasrec_align.stage1_only is False
    beauty = load_config("configs/emorecagent_align.yaml")
