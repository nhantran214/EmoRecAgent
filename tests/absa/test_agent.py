...
def test_agent_falls_back_to_classical_on_llm_error() -> None:
    tool = MockClassicalAbsaTool(
        [
            AbsaTriple(
                aspect="fit",
    ...
    agent = HybridAbsaAgent(
        tool, LLMClient(_Fail(), max_retries=1, backoff_s=0.0), min_confidence=0.5
    )
    out = agent.process("great fit")
    assert len(out.triples) == 1
    assert out.triples[0].aspect == "fit"
    assert agent.stats.llm_fallbacks == 1
