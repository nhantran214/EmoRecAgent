...
def build_emorec_graph(deps: GraphDeps) -> Any:
    ...
    def reasoning_node(state: EmoRecState) -> EmoRecState:
        ...
        existing_order = state.get("ranked_pool_order")
        price_lookup = state.get("item_prices")

        if existing_order:
            pool = list(state.get("candidate_pool") or existing_order)
            breakdowns = dict(state.get("breakdowns") or {})
            order = deps.reasoning.repair_ranked_pool(
                list(existing_order),
                pool,
                breakdowns,
            ...
            rec_ids = order[: deps.top_k]
            return {
                "ranked_pool_order": order,
        ...
        return {
            ...
            "ranked_pool_order": result.ranked_pool_order,
