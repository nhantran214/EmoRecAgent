...
@dataclass(frozen=True, slots=True)
class UserBatchEvalCase:
    """One user-level eval unit (baseline Protocol B ``user_batch``)."""

    user_local: int
    relevant_locals: frozenset[int]
    history: tuple[tuple[int, int], ...]
    n_target_rows: int
...
def _normalize_user_events(
    ...
    *,
    ...
    if not times:
        return []
    time_scale = compute_time_scale(times, time_unit_seconds=time_unit_seconds)
    time_min = min(times)
...
def build_user_sequences(
    ...
    *,
    ...
    for uid, events in per_user.items():
        ...
        if u_local is None:
            continue
        seq = _normalize_user_events(
            events, id_maps, time_unit_seconds=time_unit_seconds
        )
        if len(seq) >= 2:
            out[u_local] = seq
    return out
...
def _normalize_events_before_query(
    events: list[tuple[int, str]],
    id_maps: IdMaps,
    *,
    before_ts: int,
    ...
    """Chronological (item_local, norm_ts) using only events strictly before ``before_ts``."""
    filtered = sorted(
        [(ts, item) for ts, item in events if ts < before_ts],
        key=lambda t: (t[0], t[1]),
    )
    return _normalize_user_events(
        filtered, id_maps, time_unit_seconds=time_unit_seconds
    )


def build_user_batch_eval_cases(
    train: list[Interaction],
    ...
    *,
    ...
    """Group target rows by user; history = train-only before max target timestamp."""
    if verified_only:
        targets = [it for it in targets if it.verified_purchase]

    train_by_user: dict[str, list[tuple[int, str]]] = defaultdict(list)
    for it in train:
        train_by_user[it.user_id].append((it.timestamp, it.item))

    grouped: dict[str, list[Interaction]] = defaultdict(list)
    for it in targets:
        grouped[it.user_id].append(it)
    ...
    for uid, rows in grouped.items():
        ...
        if u_local is None:
            continue
        query_ts = max(it.timestamp for it in rows)
        history = _normalize_events_before_query(
            train_by_user.get(uid, []),
            id_maps,
            before_ts=query_ts,
        ...
        for it in rows:
            i_local = id_maps.item_to_idx.get(it.item)
            if i_local is not None:
                relevant.add(i_local)
        if not relevant:
            continue
        cases.append(
            UserBatchEvalCase(
                user_local=u_local,
                relevant_locals=frozenset(relevant),
                history=tuple(history),
                n_target_rows=len(rows),
            )
        )
    cases.sort(key=lambda c: c.user_local)
    return cases
        ...
        for item_local, norm_ts in reversed(history[:-1]):
            ...
            if nxt != 0:
                for k in range(num_negatives):
                    drawn = random_neq(1, item_num + 1, seen)
                    if num_negatives > 1:
                        neg[idx, k] = drawn
                    else:
                        neg[idx] = drawn
